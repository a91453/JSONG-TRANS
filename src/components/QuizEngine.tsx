...
export function QuizEngine({ onBack }: QuizEngineProps) {
  ...
  const generateQuestions = () => {
    ...
    const generated = selected.map((word) => {
      ...
      const options = [...wrongOptions.map(text => ({ text, id: crypto.randomUUID() })), { text: word.translation, id: correctId }].sort(() => 0.5 - Math.random());
      const correctIdx = options.findIndex(o => o.id === correctId);
      return { word, options: options.map((o, index) => ({ text: o.text, index })), correctIndex: correctIdx };
    });
      ...
      <div className={cn(containerClass, "space-y-10 py-4 animate-in fade-in duration-500")}>
        ...
        <div className={cn(centeredClass, "space-y-8")}>
          ...
          <div className="text-center space-y-3">
            <h1 className={cn("font-headline font-bold text-primary", onBack ? "text-3xl" : "text-4xl")}>{onBack ? "綜合單字能力測驗" : "綜合測驗"}</h1>
